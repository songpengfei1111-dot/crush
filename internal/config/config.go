...
type TraceAgentOptions struct {
	Model SelectedModelType `json:"model,omitempty" jsonschema:"description=Model type used by the trace execution sub-agent,enum=large,enum=small,default=large"`
	// BinaryPath points to the large-text-viewer executable used by the trace sub-agent.
	BinaryPath string `json:"binary_path,omitempty" jsonschema:"description=Path to the large-text-viewer executable used by the trace execution sub-agent"`
	// Allowed built-in tool names for the trace execution sub-agent.
	AllowedTools []string `json:"allowed_tools,omitempty" jsonschema:"description=Built-in tool names available to the trace execution sub-agent,example=view,example=grep"`
	// Allowed MCP tool sets for the trace execution sub-agent.
	AllowedMCP map[string][]string `json:"allowed_mcp,omitempty" jsonschema:"description=MCP tools available to the trace execution sub-agent"`
}
...
type Options struct {
	...
	TraceAgent                *TraceAgentOptions `json:"trace_agent,omitempty" jsonschema:"description=Options for the trace execution sub-agent"`
